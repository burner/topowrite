...
//pragma(msg, buildLogFunction(false, false, true, LogLevel.fatal));
...
abstract class Logger
{
    ...
    */
	...
	/** Default constructor for $(D Logger).
	
	The default name of the $(D Logger) will be an empty string, the default
	$(D LogLevel) will be $(D LogLevel.trace).
	*/
	public this() @safe {
		this("", LogLevel.trace);
	}
...
unittest
{
    ...
    testFuncNames(tl1);
    assert(tl1.func == "logger.testFuncNames", tl1.func);
    assert(tl1.prettyFunc == "void logger.testFuncNames(Logger logger)", 
        tl1.prettyFunc);
